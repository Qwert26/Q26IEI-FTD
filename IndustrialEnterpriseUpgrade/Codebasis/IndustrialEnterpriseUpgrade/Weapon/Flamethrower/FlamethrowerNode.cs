...
public class FlamethrowerNode : GovernedNode<FlamethrowerFiringPiece> {
	...
	private static Gradient flameGradientColor;
	...
	public Color FlameColor { get; private set; }
	static FlamethrowerNode() {
		flameGradientColor = new Gradient {
			alphaKeys = new GradientAlphaKey[] {
			new GradientAlphaKey(1, 1),
			new GradientAlphaKey(1, 0)
		},
			colorKeys = new GradientColorKey[] {
			new GradientColorKey(Color.red,1),//45° Cone
			new GradientColorKey(new Color(1,0.5f,0),0.5f),//22.5° Cone
			new GradientColorKey(new Color(1,1,0),0.25f),//11.25° Cone
			new GradientColorKey(Color.cyan,1f/7f)//6.42° Cone
		},
	...
	public override void AfterSendingOutFeelers() {
		...
		lock (flameGradientColor) {
			FlameColor = flameGradientColor.Evaluate(1f / (1f + Nozzles));
		}
	}
}